...
SMTP 발송을 확인하려면 `PUBLIC_APP_URL`, `SMTP_HOST`, `SMTP_PORT`, `MAIL_FROM`, Resend가 발급한 MAIL FROM SPF 호스트를 담은 `MAIL_SPF_DOMAIN`, 쉼표로 구분한 `MAIL_DKIM_SELECTORS`, `RESEND_WEBHOOK_SECRET`을 설정하고 `SMTP_USER=resend`, `SMTP_PASSWORD=<제한된 Resend API 키>`를 입력합니다. 587 포트는 `SMTP_SECURE=false`, `SMTP_REQUIRE_TLS=true`를 사용합니다. `notify.handol-edu.com`을 Resend에 등록하면 기본 MAIL FROM 호스트는 `send.notify.handol-edu.com`이며, 그 호스트에는 Resend가 안내한 MX와 SPF TXT를 등록합니다. SPF TXT는 정확히 하나이고 마지막 `all` 메커니즘은 `~all` 또는 `-all`이어야 합니다. 각 `{DKIM 선택자}._domainkey.{발신도메인}`은 Resend가 지정한 CNAME을 거쳐 비어 있지 않은 `v=DKIM1` 공개키로 끝나야 하고, `_dmarc.{발신도메인}`에는 `p=quarantine` 또는 `p=reject` 정책이 필요합니다. 레거시 `pct` 태그를 유지한다면 반드시 `pct=100`이어야 합니다. API 빌드 후 `npm --prefix server run preflight:production`은 SMTP DNS·TLS·인증과 모든 DKIM 선택자를 실시간 확인하며 테스트 메일은 보내지 않습니다. 결과에는 DNS 원문 대신 DMARC 정책과 발신 도메인·DKIM 선택자 목록·정규화된 레코드 집합의 SHA-256을 기록하므로, 인수 증빙에서 DNS 변경 여부를 대조할 수 있습니다. 단일 `MAIL_DKIM_SELECTOR`는 이전 설정 호환용으로만 읽습니다.
