...
SMTP 발송을 확인하려면 `PUBLIC_APP_URL`, `SMTP_HOST`, `SMTP_PORT`, `MAIL_FROM`, Resend가 발급한 MAIL FROM SPF 호스트를 담은 `MAIL_SPF_DOMAIN`, 쉼표로 구분한 `MAIL_DKIM_SELECTORS`, `RESEND_WEBHOOK_SECRET`을 설정하고 `SMTP_USER=resend`, `SMTP_PASSWORD=<제한된 Resend API 키>`를 입력합니다. 587 포트는 `SMTP_SECURE=false`, `SMTP_REQUIRE_TLS=true`를 사용합니다. `notify.handol-edu.com`의 사용자 지정 Return-Path는 `rsend.notify.handol-edu.com`이며, Resend가 발급한 CNAME은 최종적으로 MX와 SPF TXT를 제공합니다. SPF TXT는 정확히 하나이고 마지막 `all` 메커니즘은 `~all` 또는 `-all`이어야 합니다. 각 `{DKIM 선택자}._domainkey.{발신도메인}`은 비어 있지 않은 `p=` 공개키를 포함해야 하며, Resend의 직접 TXT 형식과 CNAME 뒤의 `v=DKIM1` 형식을 모두 지원합니다. `_dmarc.{발신도메인}`에는 `p=quarantine` 또는 `p=reject` 정책이 필요합니다. 레거시 `pct` 태그를 유지한다면 반드시 `pct=100`이어야 합니다. API 빌드 후 `npm --prefix server run preflight:production`은 SMTP DNS·TLS·인증과 모든 DKIM 선택자를 실시간 확인하며 테스트 메일은 보내지 않습니다. 결과에는 DNS 원문 대신 DMARC 정책과 발신 도메인·DKIM 선택자 목록·정규화된 레코드 집합의 SHA-256을 기록하므로, 인수 증빙에서 DNS 변경 여부를 대조할 수 있습니다. 단일 `MAIL_DKIM_SELECTOR`는 이전 설정 호환용으로만 읽습니다.

Resend 콘솔에는 `POST https://handol-edu.com/api/v1/mail/webhooks/resend`를 등록하고 `email.bounced`를 구독합니다. 서버는 원문 본문과 `svix-id`·`svix-timestamp`·`svix-signature`를 `RESEND_WEBHOOK_SECRET`으로 검증하고, `bounce.type=Permanent`인 이벤트의 `data.message_id`로 계정 메일 또는 문의 알림 작업을 찾아 멱등하게 `BOUNCED`로 바꿉니다. 다른 정상 서명 이벤트는 재전송을 막기 위해 성공 응답으로 무시합니다. 감사로그에는 반송 유형과 `svix-id`의 SHA-256만 기록합니다. 기존 `POST /api/v1/mail/webhooks/bounce`와 `MAIL_BOUNCE_WEBHOOK_SECRET`은 정규화 게이트웨이 호환용이며 Resend를 직접 연결하지 않습니다. 최초 상태 변경 응답의 `auditLogId`와 `eventIdSha256`을 보관하고, 운영 프리플라이트 JSON·웹훅 응답 JSON을 `npm --prefix server run verify:mail-operations`로 교차 검증하면 비식별 `mail-operations-evidence.json`을 생성할 수 있습니다. 이때 공급자 이벤트 ID 입력에는 시험 요청의 `svix-id`를 사용합니다. 수신자 주소, `svix-id` 원문과 공급자의 원문 오류는 감사로그와 증빙에 저장하지 않습니다. 계정 API와 문의 답변 API는 메일 서버 응답을 기다리지 않으며 발송·반송 결과는 감사로그에서 확인합니다. 문의 알림에는 문의 제목·본문·답변을 포함하지 않고 `PUBLIC_APP_URL`의 본인 문의함 링크만 제공합니다.
