...
현재 구현은 `GuardianInvitation(PENDING)`을 대기 연결로 사용하고 수락 시에만 `GuardianLink(ACTIVE)`를 생성합니다. 초대 토큰 원문은 데이터베이스에 저장하지 않으며, 보호자 로그인 이메일이 초대 이메일과 일치해야 수락할 수 있습니다. 개발·테스트 환경에서는 메일 발송 전 흐름 검증을 위해 응답의 `developmentToken`으로 토큰을 제공하지만 운영 응답에서는 제외합니다. 운영 메일 프리플라이트는 SPF·Resend의 모든 DKIM CNAME·DMARC와 SMTP 연결을 검사하고, DMARC 정책과 발신 도메인·DKIM 선택자 목록·정규화된 DNS 레코드 집합의 SHA-256을 남겨 원문 공개 없이 인수 시점의 DNS 관측값을 고정합니다. 서명된 `POST /mail/webhooks/resend`는 Resend의 영구 반송을 Message-ID로 연결해 계정·문의 메일을 `BOUNCED`로 기록합니다. 최초 상태 변경 응답은 수신자나 메시지 ID를 복제하지 않고 생성된 `auditLogId`와 `svix-id`의 SHA-256을 반환하므로 공급자 시험 이벤트와 내부 감사기록을 암호학적으로 대조할 수 있습니다. 실제 운영 전에는 발신 도메인 DNS와 Resend 웹훅을 연결해 현장 검증해야 합니다.
...
운영에서는 Nodemailer SMTP 어댑터가 `PUBLIC_APP_URL` 기반 인증·재설정 링크를 HTML과 텍스트 본문으로 발송합니다. 계정 토큰과 `AccountMailJob`은 같은 트랜잭션으로 생성하며 토큰 원문은 `ACCOUNT_MAIL_ENCRYPTION_KEY_BASE64`의 AES-256-GCM 암호문으로만 아웃박스에 저장합니다. 독립 워커가 원자적으로 작업을 선점하고 발송 직전에 토큰 만료·소비 여부와 계정 상태를 다시 확인한 뒤 최대 5회 재시도합니다. 성공·건너뜀·실패·영구 반송 결과는 토큰·이메일을 제외한 `EmailDelivery` 감사로그로 기록합니다. Resend 운영에는 `SMTP_HOST=smtp.resend.com`, `SMTP_PORT=587`, `SMTP_USER=resend`, 제한된 API 키인 `SMTP_PASSWORD`, `MAIL_FROM`, `PUBLIC_APP_URL`, Resend MAIL FROM 호스트인 `MAIL_SPF_DOMAIN`, 모든 선택자를 담은 `MAIL_DKIM_SELECTORS`, `RESEND_WEBHOOK_SECRET`이 필수이고 `SMTP_REQUIRE_TLS=true`, `SMTP_SECURE=false`를 사용합니다. `MAIL_BOUNCE_WEBHOOK_SECRET`은 기존 정규화 게이트웨이를 함께 운영할 때만 선택적으로 설정합니다.
...
`PAYMENT_STATUS_CHANGED` 웹훅은 본문 상태를 직접 적용하지 않고 `paymentKey`로 토스 원본을 다시 조회해 승인·실패·취소를 복구합니다. 알 수 없는 주문과 다른 이벤트는 성공 응답으로 무시해 불필요한 재전송을 막습니다. 교재·구독 웹훅은 같은 클라이언트 IP 기준으로 합산해 분당 120회로 제한하며, 초과 시 `429 TOSS_PAYMENT_WEBHOOK_RATE_LIMITED`와 `Retry-After`를 반환해 외부 결제 조회와 데이터베이스 자원 고갈을 방지합니다. 운영자 전액 환불은 주문별 고정 멱등키를 사용하며 원본의 누적 취소금액이 주문금액과 같을 때만 주문을 취소 상태로 확정합니다.
