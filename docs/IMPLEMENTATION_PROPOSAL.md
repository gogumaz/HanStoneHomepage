...
- **구현됨**: 서버 가격 기준 구독 주문, 토스 결제 승인·조회 검증, 멱등 구독 발급, 주문·구독 내역, React `/subscriptions`
- **구현됨**: 토스 웹훅 재검증·실패/취소 동기화, 운영자 전액 환불, 부분 환불 누적과 전액 환불 시 권한 회수
...
- **구현됨**: 관리자 결제 대사·불일치 표시·토스 수동 동기화·전액 환불 화면
...
- 토스페이먼츠 JavaScript SDK v2 결제 요청
- `paymentKey` 결제 승인·조회 및 주문번호·금액·상태 검증
...
DB, OAuth, 토스페이먼츠 Secret은 환경별로 완전히 분리합니다. Secret은 GitHub Secrets
...
OAuth Callback과 토스페이먼츠 웹훅은 API 도메인 아래 고정 경로를 사용합니다.
