...
| 3 | 릴리스 준비 감사 | 2026-09-11 읽기 전용 재감사에서 후보 게시·깨끗한 작업 트리·기존 필수 워크플로 8개·`production` 환경·단독 운영자·리뷰·`main` 배포 정책은 통과함. 2026-09-13 CodeQL을 아홉 번째 필수 워크플로로 추가했으며, Dependabot 주간 업데이트 정책도 웹·API·공통 컴포넌트·컨테이너·Actions 전체에 적용함. 남은 실패 조건은 저장소 Secret 6개와 production Secret 9개 및 새 워크플로의 원격 활성 확인임 | 변경 게시 후 CodeQL 첫 분석을 통과시키고 Dependabot 경고·보안 업데이트 저장소 설정을 활성화한 뒤, 최소 권한 `RELEASE_READINESS_TOKEN`과 실제 스테이징·격리 복구·메일 반송·법무 승인 자료를 준비해 Secret 15개를 등록하고 공식 감사를 재실행 |
