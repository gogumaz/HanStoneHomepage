...
export function NotificationsPage() {
  ...
  return <main className="catalog-page notifications-page">
    <header className="catalog-header"><Link className="back-link" to="/">← 개발 현황으로</Link><p className="react-stack-eyebrow">MY NOTIFICATIONS</p><h1>알림함</h1><p>문의 답변과 학급 과제 안내를 계정별로 확인합니다.</p></header>
    ...
    {me.data ? <>
      ...
      <ul className="notification-list">{list.data?.items.map((item) => <li key={item.id} data-read={Boolean(item.readAt)}>
        ...
        <div className="notification-actions">{!item.readAt ? <button type="button" disabled={read.isPending} onClick={() => read.mutate(item.id)}>읽음 처리</button> : null}{item.kind === 'inquiry_answered' ? <a href={`/board.html?type=inquiry&id=${encodeURIComponent(item.resourceId)}`} onClick={(event) => void openInquiry(event, item)}>문의 답변 열기</a> : item.resourceType === 'OrganizationClassAssignment' ? <Link to="/dashboard">과제 열기</Link> : null}</div>
      </li>)}</ul>
